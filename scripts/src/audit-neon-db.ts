...
const NEON = process.env.NEON_DATABASE_URL ?? process.env.SOURCE_DATABASE_URL ?? process.env.TARGET_DATABASE_URL;
if (!NEON) {
  console.error("Falta NEON_DATABASE_URL, SOURCE_DATABASE_URL o TARGET_DATABASE_URL");
  process.exit(1);
}
...
async function main() {
  const client = new pg.Client({ connectionString: NEON, ssl });
  try {
    await client.connect();
  } catch (e) {
    console.error("No se pudo conectar a Neon:", (e as Error).message);
    process.exit(1);
  }
  console.log("Neon DB conectada\n");
